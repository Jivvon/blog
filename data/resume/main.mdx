...
의사결정, 정책 설계, 운영 환경 개선, 기존 시스템에 대한 퍼포먼스 및 오류 개선
...
- 기술 부채 및 개선 인식
    - 현재 구조에서는 DynamoDB Streams 이벤트 유실을 방지하기 위해 병렬 구독 방식을 채택했으나, 이벤트를 보존하거나 재처리하는 구조는 미흡함
...
- 통신사로부터 SIP 통신을 위해서는 inbound/outbound가 모두 동일한 ip와 port여야 한다는 조건이 있었음
