...
### 1. Kubernetes 환경 WebRTC 묵음 장애 원인 규명 및 방어 로직 구현

> 트러블슈팅 과정을 작성한 기술 블로그: [https://channel.io/ko/team/blog/articles/1dc56e4a](https://channel.io/ko/team/blog/articles/1dc56e4a)

**[현상]**

- WebRTC 서버 스케일링 과정에서 약 25%의 통화에서 묵음 현상 간헐적 발생.
- Node Exporter의 `node_netstat_Udp_OutDatagrams` 메트릭을 모니터링한 결과, 세션은 연결되었으나 아웃바운드 패킷이 '0'인 상태임을 확인.

**[원인분석]**

- AWS EIP(Elastic IP) 할당 API 응답 시간과 실제 네트워크 라우팅이 열리는 시간 사이에 미세한 '시간차'가 존재할 것이라는 가설 수립.
- 마이크로초 단위로 NTP 동기화된 두 대의 서버를 활용해 정밀 측정 환경 구성. 타겟 EC2에서 `tcpdump`를 실행하고, 클라이언트에서 EIP 할당 API 성공 응답(T1) 직후 0.01초 간격으로 Ping 패킷을 전송하여 최초 패킷 도달 시간(T2)을 측정.
- 10회 반복 테스트 결과, API 성공 응답 후 실제 인바운드 트래픽이 뚫리기까지 **평균 1초 내외의 지연(Time gap)**이 발생함을 정량적으로 증명.
- 이 지연 시간 동안 애플리케이션이 너무 빨리 기동하여 외부 통신을 시도하면서 RTP 통신 경로가 형성되지 못하는 상태 불일치(State Mismatch)가 문제의 근본 원인임을 규명.

**[해결 및 성과]**

- 단순 Sleep 대기 방식이 아닌, 확정적인 네트워크 연결을 보장하는 방어 로직 구현.
- AWS CLI의 `associate-address --no-allow-reassociation` 옵션을 활용하고, EIP Pool에 부여된 AWS Tag를 루프 돌며 정상 부착 여부를 명확히 검증한 후 애플리케이션이 통신을 시작하도록 개선.
- 해당 로직을 프로덕션 환경에 당일 배포하여 묵음 오류 발생률을 0%로 근절하고 서비스 안정성 확보.

### 2. EKS 마이그레이션 중 AWS SDK 초기화 오류(IRSA) 딥다이브 및 해결

**[현상]**

- ECS에서 EKS로 애플리케이션을 이전하는 과정에서, EKS 환경(IRSA 적용)에서만 특정 애플리케이션이 권한을 획득하지 못하고 `AccessDenied` 예외가 발생.
- 개발팀에서 원인을 찾지 못해 마이그레이션이 블로킹된 상황.

**[원인분석]**

- CloudTrail 로그를 분석하여 Pod가 명시된 Role을 Assume하려는 이벤트 자체가 실패하고 있음을 1차 확인.
- 인프라 레이어를 넘어 애플리케이션의 AWS SDK 내부 동작 로직(Credential Provider Chain)을 코드 레벨에서 직접 디버깅.
- 애플리케이션에서 사용하는 Custom Endpoint Resolver가 STS 요청을 덮어쓰고 있었으며, 명시적으로 설정하지 않은 옵션은 기본값(Default)조차 주입되지 않는 SDK 내부의 엣지 케이스를 발견. 이로 인해 기존에 의존하던 필수 옵션이 누락되어 STS 권한 획득이 실패함을 규명.

**[해결 및 성과]**

- 누락된 필수 옵션을 명시적으로 주입하도록 애플리케이션 코드를 수정하여 문제 즉각 해결.
- 인프라 엔지니어로서 애플리케이션 레이어의 버그까지 파고들어 동료 개발자들의 장기 미해결 이슈를 해소하고, EKS 마이그레이션 일정을 차질 없이 추진.
